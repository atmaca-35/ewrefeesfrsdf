document.addEventListener('DOMContentLoaded', async () => {
    ...
    function searchWord(query) {
        ...
        if (query.trim().length === 0 || query.startsWith(' ')) {
            // Empty or invalid search should trigger the error state
            searchContainer.classList.add('error');
            ghostText.textContent = "";
            return;
        } else {
            searchContainer.classList.remove('error'); // Valid search removes error state
        }

        const normalizedQuery = normalizeTurkish(query);

        // Exclude specialWords and clickableWords from the search
        const searchableData = Object.keys(dictionaryData)
            .filter(key => key !== 'specialWords' && key !== 'clickableWords');

        const sortedWords = searchableData
        ...
        if (closestWord) {
            ...
            // Handle ghostText with spaces correctly
            const remainingQuery = closestWord.word.substring(query.length);
            if (remainingQuery.startsWith(' ')) {
                ghostText.textContent = ' ' + remainingQuery.trim();
            } else {
                ghostText.textContent = remainingQuery;
            }
        } else {
            // No result found, mark as error
            ghostText.textContent = "";
            searchContainer.classList.add('error');
        }
    ...
    function updateSearchBoxPlaceholder(query) {
        ...
        const matchingWord = searchableData
            .map(word => ({ word: normalizeTurkish(word), original: word }))
            .sort((a, b) => a.word.localeCompare(b.word))
            .find(({ word }) => word.startsWith(queryLower));

        if (matchingWord) {
            const remainingPart = matchingWord.original.substring(query.length);
